...
class DeviceTunnel extends events.EventEmitter {
  ...
  static createFor(device, verbose=false, controlTimeout=5) {

    const getZoneAttrs = Promise.promisify(device.getZoneAttrs.bind(device));

    return getZoneAttrs().then((zoneAttrs) => {
      return new DeviceTunnel(device, zoneAttrs.CurrentZoneName, verbose, controlTimeout);
    });
  }

  constructor(device, deviceName, verbose, controlTimeout) {
  ...
  bindAirplayServer(verbose, control) {

    console.log('found some shit', this.deviceName);
